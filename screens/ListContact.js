...
export default function ListContact({ navigation }) {
  ...
  useEffect(async () => {
    ...
    console.log("ggggggg" + userData?.Data._id);
    const response = await fetch(
      `http://192.168.43.116:5000/contactsUtilisateur/${userData.Data._id}`
    );
    const jsonData = await response.json();
    setData(jsonData);
    console.log("bbbbboumaima" + jsonData);
  }, []);
  ...
  const deleteContact = async (id) => {
    try {
      await fetch(`http://192.168.43.116:5000/api/deleteC/${id}`, {
        method: "DELETE",
      });
      Toast.show({
        type: "error",
          ...
          >
            ...
            <View style={{ flexDirection: "row" }}>
              <Text style={{ marginStart: 10 }}>
                Spécialité: {item.Specialite_docteur}
              </Text>
              <TouchableOpacity
                onPress={() => {
              ...
              >
                <AntDesign
                  ...
                  size={27}
                  color="blue"
                  style={{ marginStart: 0, marginRight: 10 }}
                />
              </TouchableOpacity>
  ...
  const filteredData = data
    ? data.filter(
        (item) =>
          item.Specialite_docteur.toLowerCase().includes(
